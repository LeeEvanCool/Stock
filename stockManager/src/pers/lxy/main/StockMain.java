...
import pers.lxy.test.StockThread;
...
public class StockMain {
	...
	public static void main(String[] args) {
		...
		if (str.equals("1")) {
			for (Map<String, String> map : stack) {
				StockThread stockThread = new StockThread(map.get("stockCode"), map.get("stockTable"));
				Thread thread = new Thread(stockThread);
				thread.start();
			}
	...
	private static List<Map<String, String>> getStackValue() {
		...
		try {
			...
			String sql = "select stock_code, stock_table from " + STOCKCODETABLE + " where is_enable = '1' ";
			...
			if (resultSet != null) {
				while (resultSet.next()) {
					...
					if (stockCode != null && stockTable != null) {
